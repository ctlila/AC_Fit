...
w = 1 ; % weight of chi2 compared to chi1 in the fitting process.
...
%Instrument choice
instr = input("ppms or squid or custom?",'s');
if isempty(instr)
    instr = 'ppms';
end
...
elseif contains(instr,'custom')
    rawfile=input("Name (and path) of your raw data file CUSTOM",'s');
    if isempty(rawfile)
        rawfile = 'C:\Users\Lila\Documents\STAGE\MESURES\KV788_Lurad_acscanH.ac.dat';
    end
end
...
if contains(instr,'custom','IgnoreCase',true)
    temperatures = Rawdata(:,2);
    amplitudes = 3.5*(ones(length(Rawdata(:,1)),1));
    fields = Rawdata(:,1);
    mpr = Rawdata(:,4);
    stddev = 1e-6*(ones(length(Rawdata(:,1)),1));
    fq = Rawdata(:,3);
    msec = Rawdata(:,5);
end
